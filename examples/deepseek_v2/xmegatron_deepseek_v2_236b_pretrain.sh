...
MEGATRON_PATH=/workspace/Megatron-LM-0.10
...
CP=1
AC=${AC:-full}
MP_AC_LAYERS=${MP_AC_LAYERS:-7}
TASK=sft # pretrain/sft
DATASET_TYPE=mmap # mmap/raw
SAVE_INTERVAL=${SAVE_INTERVAL:-5}
PRETRAIN_CHECKPOINT_PATH=${PRETRAIN_CHECKPOINT_PATH:-"/mnt/geogpt-training/home/qianhao/models/megatron_ckpt/021-236b-A21b-sft-pp8-ep8"}
DATASET_PATH=${DATASET_PATH:-"/mnt/geogpt-training/home/qianhao/data/sft_sample_8k_text_document"}
...
DETAIL_TASK_NAME="${TASK_NAME}-lr-${LR}-minlr-${MIN_LR}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}-pr-${PR}-tp-${TP}-pp-${PP}-cp-${CP}-virtual_pp-${VIRTUAL_PP}-ac-${AC}-do-${DO}-sp-${SP}-ep-${EP}/${CURRENT_TIME}"
...
#not need --auto-detect-ckpt-format
